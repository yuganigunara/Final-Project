...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    // Form has been submitted, set $form_submitted to true
    $form_submitted = true;
    ...
    $sql = "INSERT INTO free_meal_plan (height, weight, age, gender, exercise, diet, allergies)
            VALUES ('$height', '$weight', '$age', '$gender', '$exercise', '$diet', '$allergies_string')";
    ...
    if (!empty($height) && !empty($weight) && !empty($age) && !empty($gender)) {
        ...
        // Define meal options (include dietary preference adjustments here)
        $meal_options = [
            'low_bmi' => [
                'breakfast' => ["Oatmeal with honey", "Banana smoothie", "Scrambled eggs", "Yogurt with fruit", "Pancakes", "Toast with avocado", "Cereal"],
                'lunch' => ["Grilled chicken with rice", "Turkey wrap", "Quinoa with veggies", "Salad with chickpeas", "Tuna salad", "Vegetable stir fry", "Grilled chicken with quinoa"],
                'dinner' => ["Grilled salmon with potatoes", "Chicken soup", "Baked cod with asparagus", "Stir-fried tofu with rice", "Grilled vegetables", "Baked chicken with sweet potatoes", "Vegetable curry"],
                'snacks' => ["Fruit salad", "Greek yogurt", "Granola bars", "Nuts", "Apple slices", "Rice cakes", "Carrot sticks"]
            ],
            'normal_bmi' => [
                'breakfast' => ["Avocado toast", "Smoothie bowl", "Whole-grain cereal with milk", "Pancakes", "Chia pudding", "Oatmeal", "Egg muffins"],
                'lunch' => ["Grilled chicken salad", "Tuna sandwich", "Vegetable stir fry", "Vegetable soup", "Quinoa salad", "Grilled chicken with veggies", "Lentil soup"],
                'dinner' => ["Baked chicken with rice", "Grilled shrimp with quinoa", "Vegetable curry", "Baked salmon", "Tofu stir fry", "Grilled pork with greens", "Vegetable stew"],
                'snacks' => ["Mixed nuts", "Protein bars", "Low-fat cheese sticks", "Fruit", "Yogurt", "Granola", "Rice cakes"]
            ],
            'high_bmi' => [
                'breakfast' => ["Vegetable omelette", "Fruit smoothie", "Whole-grain toast with peanut butter", "Chia pudding", "Green smoothie", "Scrambled eggs", "Greek yogurt with nuts"],
                'lunch' => ["Vegetable soup with chicken", "Grilled fish with steamed veggies", "Salad with quinoa", "Vegetable salad with chickpeas", "Lentil stew", "Grilled tofu with vegetables", "Chicken breast with avocado"],
                'dinner' => ["Steamed fish with greens", "Chicken breast with spinach", "Vegetable stew", "Grilled chicken with broccoli", "Vegetable curry", "Steamed vegetables with tofu", "Grilled steak with spinach"],
                'snacks' => ["Carrot sticks", "Apple slices", "Low-calorie yogurt", "Hummus with vegetables", "Cucumber slices", "Almonds", "Celery sticks"]
            ]
        ];

        // Define allergens
        $allergen_foods = [
            'nuts' => ['granola bars', 'peanut butter', 'mixed nuts'],
            'dairy' => ['yogurt', 'cheese sticks', 'whole-grain cereal with milk'],
            'gluten' => ['whole-grain toast', 'turkey wrap', 'protein bars'],
        ];

        // Adjust meal options based on allergies
        foreach ($meal_options as &$meal_category) {
            foreach (['breakfast', 'lunch', 'dinner', 'snacks'] as $meal_type) {
                $meal_category[$meal_type] = array_filter($meal_category[$meal_type], function($meal) use ($allergies, $allergen_foods) {
                    foreach ($allergies as $allergy) {
                        if (in_array(strtolower($meal), $allergen_foods[$allergy] ?? [])) {
                            return false;
                        }
                    }
                    return true;
        ...
        // Adjust meal options based on dietary preferences
        if ($diet === "vegan") {
            foreach ($meal_options as &$meal_category) {
                $meal_category['breakfast'] = array_filter($meal_category['breakfast'], fn($meal) => !str_contains($meal, "egg"));
                $meal_category['lunch'] = array_filter($meal_category['lunch'], fn($meal) => !str_contains($meal, "chicken"));
                $meal_category['dinner'] = array_filter($meal_category['dinner'], fn($meal) => !str_contains($meal, "fish"));
            }
        }
        ...
        $bmi_category = '';
        if ($bmi < 18.5) {
            $bmi_category = 'low_bmi';
        } elseif ($bmi >= 18.5 && $bmi <= 24.9) {
            $bmi_category = 'normal_bmi';
        } else {
            $bmi_category = 'high_bmi';
        }

        // Generate meal plan in table format
        $selected_meals = $meal_options[$bmi_category];
        $meal_plan = "<h3>Your Personalized Weekly Meal Plan</h3>";
