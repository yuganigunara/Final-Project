...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $sql = "INSERT INTO paidmeal2 (height, weight, age, gender, exercise, diet, allergies, pregnant, meal_plan_type, meal_preference, delivery_address, delivery_time, delivery_days, custom_meal_plan)
    ...
    if (!empty($height) && !empty($weight) && !empty($age) && !empty($gender)) {
        ...
        $meal_options = [
            'low_bmi' => [
                'breakfast' => ["Oatmeal with honey", "Banana smoothie", "Scrambled eggs", "Yogurt with fruit"],
                'lunch' => ["Grilled chicken with rice", "Turkey wrap", "Quinoa with veggies"],
                'dinner' => ["Grilled salmon with potatoes", "Chicken soup", "Baked cod with asparagus"],
                'snacks' => ["Fruit salad", "Greek yogurt", "Granola bars"]
            ],
            'normal_bmi' => [
                'breakfast' => ["Avocado toast", "Smoothie bowl", "Whole-grain cereal with milk"],
                'lunch' => ["Grilled chicken salad", "Tuna sandwich", "Vegetable stir fry"],
                'dinner' => ["Baked chicken with rice", "Grilled shrimp with quinoa", "Vegetable curry"],
                'snacks' => ["Mixed nuts", "Protein bars", "Low-fat cheese sticks"]
            ],
            'high_bmi' => [
                'breakfast' => ["Vegetable omelette", "Fruit smoothie", "Whole-grain toast with peanut butter"],
                'lunch' => ["Vegetable soup with chicken", "Grilled fish with steamed veggies", "Salad with quinoa"],
                'dinner' => ["Steamed fish with greens", "Chicken breast with spinach", "Vegetable stew"],
                'snacks' => ["Carrot sticks", "Apple slices", "Low-calorie yogurt"]
            ]
        ];
        ...
        if ($form_submitted && $bmi_category) {
            $meal_plan .= "<h3>Your Personalized 7-Day Meal Plan</h3>";
        
            $meal_plan .= "<table border='1' cellpadding='10' cellspacing='0' style='border-collapse: collapse; width: 100%; margin: 20px 0;'>
                <thead style='background-color:rgb(29, 49, 70); color: white;'>
                    <tr>
                        <th style='border: 1px solid #ddd; text-align: center; padding: 10px;'>Meal Type</th>
                        <th style='border: 1px solid #ddd; text-align: center; padding: 10px;'>Day 1</th>
                        <th style='border: 1px solid #ddd; text-align: center; padding: 10px;'>Day 2</th>
                        <th style='border: 1px solid #ddd; text-align: center; padding: 10px;'>Day 3</th>
                        <th style='border: 1px solid #ddd; text-align: center; padding: 10px;'>Day 4</th>
                        <th style='border: 1px solid #ddd; text-align: center; padding: 10px;'>Day 5</th>
                        <th style='border: 1px solid #ddd; text-align: center; padding: 10px;'>Day 6</th>
                        <th style='border: 1px solid #ddd; text-align: center; padding: 10px;'>Day 7</th>
                    </tr>
                </thead>
                <tbody>";
        
            $colors = ['#ffeb3b', '#8bc34a', '#03a9f4', '#f44336'];  // Color array for alternating rows
            
            // Loop through each meal type (breakfast, lunch, dinner, snacks)
            foreach (['breakfast', 'lunch', 'dinner', 'snacks'] as $meal_type) {
                $meal_plan .= "<tr style='background-color: " . array_shift($colors) . ";'>
                                <td style='border: 1px solid #ddd; padding: 10px; background-color: #f5f5f5; font-weight: bold;'>$meal_type</td>";
        
                // For each day, fill in the corresponding meals
                for ($day = 1; $day <= 7; $day++) {
                    $meal_plan .= "<td style='border: 1px solid #ddd; padding: 8px; text-align: center; background-color: #fff;'>"
                                    . $meal_options[$bmi_category][$meal_type][($day - 1) % count($meal_options[$bmi_category][$meal_type])] 
                                    . "</td>";
                }
        
                $meal_plan .= "</tr>";
                array_push($colors, end($colors));  // Re-add the color back for the next row
            }
        
            $meal_plan .= "</tbody></table>";
        
            // AI-based meal advice
